'''
Loads data from the CVC-ClinicDB dataset for training and validation.
...
class CVCClinicDBDataset(Dataset):
    def __init__(self, root_dir, indices=None, augmentation=None):
        ...
        self.image_dir = os.path.join(root_dir, 'Data', 'CVC-ClinicDB', 'Original')
        self.mask_dir = os.path.join(root_dir, 'Data', 'CVC-ClinicDB', 'Ground Truth')
...
def load_cvc_clinicdb_data(root_dir, batch_size=32, shuffle=True, num_workers=1, augmentations=[None], seed=42):
    # Load the dataset
    full_dataset = CVCClinicDBDataset(root_dir=root_dir)
    ...
    validation_dataset = CVCClinicDBDataset(
        root_dir=root_dir, 
